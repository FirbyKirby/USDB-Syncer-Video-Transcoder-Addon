...
class BackupRestoreProgressDialog(QtWidgets.QDialog):
    """Dialog showing progress of backup restoration."""
    canceled = Signal()
    ...
    def _setup_ui(self) -> None:
        ...
        self.lbl_status = QtWidgets.QLabel(f"Restoring backup 0 of {self.total}...")
        ...
        self.details_group = QtWidgets.QGroupBox("Currently restoring")
        ...
        self.lbl_stats = QtWidgets.QLabel("Restored: 0 | Failed: 0")
    ...
    def update_progress(self, current: int, total: int, backup: BackupInfo) -> None:
        ...
        if backup.restore_status == "restored":
            self._restored += 1
        elif backup.restore_status == "failed":
            self._failed += 1
            
        self.lbl_stats.setText(
            f"Restored: {self._restored} | Failed: {self._failed}"
        )
